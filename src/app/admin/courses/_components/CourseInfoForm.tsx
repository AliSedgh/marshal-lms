...
import { Loader2, SaveIcon, SparkleIcon } from "lucide-react";
  ...
  const form = useForm<CourseSchemaType>({
    ...
    defaultValues: {
      ...
      category: (course?.category ||
        "Health & Fitness") as CourseSchemaType["category"],
  ...
  const onSubmit = (data: CourseSchemaType) => {
    startTransition(async () => {
      const result = await tryCatch(
        courseId ? editCourse(courseId, data) : createCourse(data),
      );
      if (result.error) {
      ...
      <Button disabled={isPending} type="submit">
        {isPending ? (
          ...
          <>
            ...
            <SaveIcon className="ml-1" size={16} />
          </>
        )}
      </Button>
    </form>
